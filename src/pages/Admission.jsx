  ...
  return (
    <>
      ...
      <section className="bg-section py-24 px-4 text-center border-b border-border">
        ...
        <h1 className="text-4xl md:text-5xl font-bold text-primary font-playfair mb-6">Admission Process</h1>
        <p className="text-text-secondary text-lg max-w-2xl mx-auto">Your journey to becoming a global doctor starts here.</p>
      </section>

      {/* Steps */}
      <section className="py-24 px-4 bg-white relative">
        <div className="max-w-5xl mx-auto">
          <div className="hidden lg:block absolute top-[50%] left-[50%] -translate-x-[50%] -translate-y-[50%] w-px h-full bg-section" />
            ...
            {steps.map((step, idx) => (
              <div key={idx} className={`flex flex-col lg:flex-row items-center gap-8 lg:gap-16 ${idx % 2 === 0 ? 'lg:flex-row-reverse' : ''}`}>
                <div className={`flex-1 w-full lg:w-1/2 ${idx % 2 === 0 ? 'lg:text-left' : 'lg:text-right'}`}>
                  <h3 className="text-2xl font-bold text-primary font-playfair mb-3">{step.title}</h3>
      ...
      <section className="bg-navy py-24 px-4 text-center">
        <div className="max-w-3xl mx-auto space-y-8">
          ...
          <p className="text-text-secondary">Our counselors are ready to guide you through every step.</p>
          <div className="flex flex-col sm:flex-row justify-center gap-4">
            ...
            <Link
              to="/fees"
                className="border border-border text-white px-8 py-4 rounded text-sm font-bold uppercase tracking-wide hover:bg-navy/50 transition-colors"
            >
              Review Fees
            </Link>
          </div>
